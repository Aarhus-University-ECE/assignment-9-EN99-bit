...
void DFT(node *root) // Implement DFS
{
  ...
  new_stack = push(new_stack, root);                    // Calling push to place the root into the stack
  ...
  while (!(isEmpty(new_stack))) // loop until stack is empty
  {
    ...
    {
      new_stack = push(new_stack, q->node->rchild); // call push function with the new stack and right child as arguments
    }
    if (q->node->lchild != NULL)
    {
      new_stack = push(new_stack, q->node->lchild); // same with left child. Push to stack when found
    }
  }
